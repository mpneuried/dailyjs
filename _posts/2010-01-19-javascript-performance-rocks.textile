...
The next chapter, _Is Your App Behaving Badly?_ helps you determine if your app has performance issues.  There's _Toolbox_ chapter exploring tools like Firebug and YSlow, and even some help for IE.  The next chapter explains how to use the DOM Monster, combining this with other tools to get into the heart of the causes of performance issues.
...
This part explains load order, caching and concatenation.  Sprockets is used as an example of a tool that does concatenation.  There is also some detailed coverage on how to set up Apache to cache properly, with solutions for gotchas like JSON and generated JavaScript.

The chapter on inlining is interesting.  It explains when it's safe to inline for performance reasons, and shows how to do it whilst retaining some level of sanity.  It also talks about pre-caching -- loading JavaScript and other resources while your user is doing a slow action.  Similarly, _Chapter 9: Reduce Complexity_ gives strategies for reducing the overheads introduced by DOM elements and browser-based solutions.

h3. Runtime

This part covers the really juicy stuff -- the part that many of the readers of this blog would think of first when looking at a book about JavaScript performance.  It covers everything from the use of operators and expression tuning, to run away timers and the advantages of JSON over XML.
...
There's also a chapter on iPhone tips and tricks.  If you're a Cocoa iPhone developer and you use WebKit to wrap content or functionality, or if you're a web developer targetting the iPhone browser, there are some interesting tips in here.

h3. UI Coping

This part gives advice on how to structure your app to perform well, even when technical fixes might not be possible.  It gives overall architectural advice, and design techniques like progress bars and loading screens.  This part is lighter on technical examples but has useful tips for getting JavaScript developers and designers to create somethng customers will enjoy to use.

h3. Conclusion

This book is a treasure trove for web app developers.  It covers the whole life-cycle: UI design, JavaScript performance, server-side and infrastructure issues.  It's written in a casual tone, it makes deeply technical aspects of JavaScript performance easy to understand.
...
*Disclaimer*: We've got affiliate links for JavaScript Performance Rocks --  although I bought my own copy with my hard-earned cash and I wouldn't post a review of something that wasn't worth buying!  Buying a copy through the affiliant link will also help support DailyJS, which currently makes use negative dollars.
