...
Following up from "last week's post about gRaphaël":http://dailyjs.com/2009/12/09/raphael-javascript-charts/, I thought I'd try to summarise some of the other charting options available to JavaScript developers.

h3. "Flot":http://code.google.com/p/flot/

Flot is a jQuery-based JavaScript plotting library, which focuses on simple usage. The results are fairly attractive, and has some interactive features, such as zooming.  It works with all modern browsers (including IE6+, where Google's "ExCanvas":http://excanvas.sourceforge.net/ is used). If you're into Prototype, check out "Flotr":http://solutoire.com/flotr/, a Prototype port.
